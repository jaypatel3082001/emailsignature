...
function BackendContainer() {
    ...
    const handTemp = (e)=>{
       ...
       data.map((ele,ind)=>{
            if(e.target.value===ele.id){
                axios.put(`https://6655cb453c1d3b60293b1f2c.mockapi.io/emailsignature/users/${userid}/emailuserdetails/${ele.id}`,ele)
                .then(res => {
    ...
    const handleEdit =(e)=>{
        data.map((ele)=>{
            if(ele.id===e.target.value){
                ...
                navigator(`/users/${ele.userId}/emailuserdetails`)
    ...
    const handleDelete= async (e)=>{
      ...
      // })
      // setData(filerarray)
    await axios.delete(`https://6655cb453c1d3b60293b1f2c.mockapi.io/emailsignature/users/${userid}/emailuserdetails/${e.target.value}`)
        .then(res => {
           setData(res.data)
           navigator(0)
        })
    ...
    const handleDup = (e) =>{
      data.map((ele)=>{
        if(ele.id==e.target.value){
          e.preventDefault()
          axios.post(`https://6655cb453c1d3b60293b1f2c.mockapi.io/emailsignature/users/${userid}/emailuserdetails`,ele)
          .then(res => {
              console.log(res)
              navigator(0)
          })
    ...
    useEffect(()=>{
      ...
      console.log("current value of the dattttttttttttttttttttttttt",data)
  ...
  <div className="wrap w-[70%] h-full mt-5" key={ind}>
    <div className="head bg-white flex flex-col">
      <span className="px-3 mt-3">{ele.template ? ele.template : "Template"}</span>
      <div className=" w-full flex p-3 border-b-[1px] border-gray-300"></div>
    </div>
    <div className="body bg-white w-full p-5">
      ...
      <div className="flex justify-between">
        ...
        <button className="mx-3 rounded-md border-[1px] border-black bg-white mt-5">
          <span className="py-1 px-3" value={ele.id} onClick={handleAction}>Action</span>
          {actionstate === ele.id && (
            <div className="absolute mt-3 bg-white p-3">
              <ul>
                <li value={ele.id} onClick={renameHand}>Rename</li>
                <li value={ele.id} onClick={handleDup}>Duplicate</li>
        ...
        {renamestate === ele.id && (
          <div className="absolute w-full h-full opacity-80 bg-black top-0 left-0 z-20">
            <div className="flex justify-center items-center mt-5">
              <div className="bg-white z-20 opacity-100 rounded-md px-1 text-black">
 ...
 <Link to={`/users/${userid}/emailuserdetails`}> <button>Create own Signature</button></Link>
)}
